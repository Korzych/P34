...
namespace lambient {
	...
	{
		bool otwarty = this->set1->serialPort1->IsOpen;
		...
		if (this->set1->serialPort1->IsOpen)
		{
			...
			while (running)
			{
				...
				if (threads)
				{
					for (int j = 0; j < 54; j++)//100
					{

						suma++;
						Kolor* ptr1 = new Kolor;
						cv1.push_back(ptr1);
						MyThread^ o1 = gcnew MyThread(sidecap, j * 20, ptr1);
						Thread^ oThread1 = gcnew Thread(gcnew ThreadStart(o1, &MyThread::ThreadProc));

					
						Kolor* ptr2 = new Kolor;
						cv2.push_back(ptr2);
						MyThread^ o2 = gcnew MyThread(-sidecap, j * 20, ptr2);
